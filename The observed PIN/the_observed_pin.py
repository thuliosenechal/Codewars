...
where we assume to find all the stolen stuff. The door to this 
warehouse is secured by an electronic combination lock. Unfortunately 
...
(horizontally or vertically, but not diagonally). E.g. instead of the 1
 it could also be the 2 or 4. And instead of the 5 it could also be the 
 2, 4, 6 or 8.
...
a function, that returns an array (or a list in Java and C#) of all 
...
def get_pins(observed):
    list1 = [adjacent[n] for n in observed]
    list2 = list(itertools.product(*list1))
    return [''.join(n) for n in list2]
